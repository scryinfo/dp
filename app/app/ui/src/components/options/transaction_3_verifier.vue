...
export default class transaction_3_verifier extends Vue{
    selectedTx= "";     // tId= ""
    curPage= 1;
    pageSize= 6;
    total= 0;
    ...
    register (pwd:string) {
        connects.send({Name:"register", Payload:{password: pwd}}, function (payload, _this) {
    ...
    vote (pwd:string) {
        connects.send({Name:"vote", Payload:{password: pwd, TransactionId: this.selectedTx, verify: this.verify}},
            function (payload, _this) {
    ...
    get listenTxVRefresh(){
        return this.$store.state.transactionverifier;
    };

    @Watch('this.$store.state.transactionverifier',{immediate:true,deep:true})
    listenTxVRefresh1 () {
        this.curPage = 1;
        this.total = this.$store.state.transactionverifier.length;
    };
