...
export default class transaction_2_buyer extends Vue{
    selectedTx: any = {};  // {tId= ""; Verifier1Response= ""; Verifier2Response= ""; SupportVerify= false}
    curPage= 1;
    pageSize= 6;
    total= 0;
    ...
    cancelPurchase (pwd:string) {
        connects.send({Name:"cancelPurchase", Payload:{password: pwd, TransactionId: this.selectedTx.TransactionId}}, function (payload, _this) {
    ...
    confirmPurchase (pwd:string) {
        connects.send({Name:"confirmPurchase", Payload:{password: pwd, TransactionId: this.selectedTx.TransactionId}}, function (payload, _this) {
    ...
    decrypt (pwd:string) {
        connects.send({Name:"decrypt", Payload:{password: pwd, TransactionId: this.selectedTx.TransactionId}}, function (payload, _this) {
    ...
    confirmDataPre (array:Array<any>) {
        this.supportVerify = array[0];
    }

    confirmData (pwd:string) {
        connects.send({Name:"confirmData", Payload:{password: pwd, TransactionId: this.selectedTx.TransactionId,
    ...
    gradePre (array:Array<any>) {
        this.verifier1Revert = array[0];
        this.verifier2Revert = array[1];
    }

    gradeToVerifier (pwd:number) {
        connects.send({Name:"gradeToVerifier", Payload:{password: pwd, TransactionId: this.selectedTx.TransactionId, grade: {
    ...
    get listenTxBRefresh(){
        return this.$store.state.transactionbuy;
    }

    @Watch('this.$store.state.transactionbuy',{immediate:true,deep:true})
    listenTxBRefresh1 () {
        this.curPage = 1;
        this.total = this.$store.state.transactionbuy.length;
    }
