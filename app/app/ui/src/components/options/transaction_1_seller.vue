...
export default class transaction_1_seller extends Vue {
    selectedTx= "";  // tId: ""
    curPage= 1;
    pageSize= 6;
    total= 0;
    ...
    reEncrypt (pwd:string) {
        connects.send({ Name:"reEncrypt", Payload:{password: pwd, TransactionId: this.selectedTx}}, function (payload:any, _this:any) {
    ...
    get listenTxSRefresh(){
        return this.$store.state.transactionsell;
    }

    @Watch('this.$store.state.transactionsell',{immediate:true,deep:true})
    listenTxSRefresh1 () {
        this.curPage = 1;
        this.total = this.$store.state.transactionsell.length;
    }
