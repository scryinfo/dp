...
export default {
    ...
    methods: {
        ...
        pub: function () {
            let _this = this
            let pub = {}
            pub.Title = this.pubData.Title; pub.Price = this.pubData.Price; pub.Keys = this.pubData.Keys
            pub.Description = this.pubData.Description; pub.Owner = this.$store.state.account
            astilectron.sendMessage({Name:"publish",Payload:this.pubData}, function (message) {
                if (message.error == null) {
                    let ID = "Qm462" // interface call will return an unique id also,remember to modify here.
                    dl_db.write(pub, ID)
                    dl_db.init(_this)
                    console.log("Node: Publish new data success.")
                }else {
