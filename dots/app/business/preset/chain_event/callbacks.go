...
func (c *Callbacks) onPublish(event event.Event) bool {
	...
	if num, err := c.DB.Read(&definition.DataList{}, "", "publish_id = ?", dl.PublishId); num == 0 { // "error": "record not found"
		if num, err := c.DB.Insert(&dl); num != 1 || err != nil {
			...
			var evt DBDef.Event
			{
				evt.NotifyTo = []byte(c.CurUser.Account().Addr)
				evt.EventName = event.Name
				evt.EventBody = []byte(event.Data.String())
				evt.EventStatus = 0
				evt.CreatedTime = time.Now().Unix()
			}
			if _, err := c.CreateEvt(&evt); err != nil {
				dot.Logger().Errorln("db insert event failed", zap.NamedError("error", err))
				return false
			}
		}
...
// Create Event
func (c *Callbacks) CreateEvt(evt *DBDef.Event) (i interface{}, err error) {

	var num int64
	if num, err = c.DB.Insert(&evt); num != 1 || err != nil {
		err = errors.Wrap(err, "in create new event")
		return
	}
