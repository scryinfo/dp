...
// As a matter of fact, two requests of Api may be responded with the same or different pieces of flight dataToCache.

// If the datum responded are the same, the last Api request is just a repeated work, which may not only contribute to Api cost,
...
// Probably, we can cache flight dataToCache out of Api, and should prefer to inquire effective dataToCache from cache before calling Api.
...
// As a compromise proposal, Api calling intervals can be set, during the validPeriod, we prefer to fetch flight value directly from cache, otherwise
// an wholly new value should be made. This solution comes with benefit that we don't have to value Api too frequently, which can save us some cost
// required by value go provider.
...
// todo: Api.call. How does Api status respond to HTTP response?
