...
func VariFlightServerTypeLives() []*dot.TypeLives {
			...
			Meta: dot.Metadata{
				...
				NewDoter: func(conf []byte) (dot.Dot, error) {
					...
					dot.Logger().Debug(func() string {
						return spew.Sprintf("VariFlightServerConfig: %#+v", _conf)
					})
	...
	typeLives = append(typeLives, gserver.WebSocketAndRelyTypeLives()...)
	return typeLives
}

func (s *VariFlightServer) AfterAllInject(l dot.Line) {
	s.grpcServer = grpc.NewServer()
	s.serviceServer = new(s.VariFlightApiCaller)
