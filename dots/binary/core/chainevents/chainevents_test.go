...
func TestUnSubscribeExternal(t *testing.T) {
	...
	mapEventSubscribe[evtName1] = subMap1
	mapEventSubscribe[evtName2] = subMap2

	unsubscribe(addr1, evtName1, repo)

	if len(mapEventSubscribe[evtName1]) != 1 {
		t.Fail()
	}

	if len(mapEventSubscribe[evtName2]) != 2 {
		t.Fail()
	}

	unsubscribe(addr2, evtName1, repo)

	if len(mapEventSubscribe[evtName1]) != 0 {
		t.Fail()
	}
}
